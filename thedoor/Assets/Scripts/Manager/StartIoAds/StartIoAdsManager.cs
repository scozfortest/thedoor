...
public class StartIoAdsManager : MonoSingletonA<StartIoAdsManager> {
    ...
    public void InitializeAds() {
        ...
        IsInit = true;
        WriteLog.Log("[StartIo Ads開始初始化] Initialize");
    ...
    private void OnRewardedVideoShowFailed(object sender, EventArgs e) {
        WriteLog.Log("[StartIo Ads顯示廣告失敗] RewardedVideo show failed");
    ...
    private void OnRaiseAdClosed(object sender, EventArgs e) {
        WriteLog.Log("[StartIo Ads廣告關閉] RewardedVideo closed");
        // 給獎勵
    ...
    private void OnRaiseAdVideoCompleted(object sender, EventArgs e) {
        WriteLog.Log("[StartIo Ads廣告展示到最後成功] RewardedVideo finished");
    }
    // Called when reward video is clicked
    private void OnRaiseAdClicked(object sender, EventArgs e) {
        WriteLog.Log("[StartIo Ads廣告被點擊成功] RewardedVideo clicked");
    }
    #endregion
    ...
    public void ShowStartIoAd(Action<bool, AdsResultMessage> onShowStartIoAdsCallBack) { // Show the loaded content in the Ad Unit:
        ...
        WriteLog.Log("[StartIo Ads顯示廣告單元] Showing Ad: ");
        ...
        if (!InitializeSuccess) {
            ...
            WriteLog.Log("[StartIo Ads顯示廣告單元] StartIoAds_Not_Initialize");
        ...
        if (!StartIoAdReadyToShow) {
            ...
            WriteLog.Log("[StartIo Ads顯示廣告單元] StartIoAdReadyToShow = false ShowStartIoAd StartIoAds_NotReady");
            OnShowStartIoAds = null;
            // 重新載入一個
            LoadInterstitial();
            return;
        ...
        if (IsShowAds) {
            ...
            WriteLog.Log("[StartIo Ads顯示廣告單元] Ads_AlreadyShowing");
            OnShowStartIoAds = null;
            return;
        }
        ...
        if (InterstitialAd.IsReady()) {
            ...
            IsShowAds = true;
            WriteLog.Log("[StartIo Ads顯示廣告單元] 開始 ShowAd");
        } else { // 沒準備好
            onShowStartIoAdsCallBack?.Invoke(false, AdsResultMessage.StartIoAds_NotReady);
            WriteLog.Log("[StartIo Ads顯示廣告單元] ShowStartIoAd StartIoAds_NotReady");
            OnShowStartIoAds = null;
            IsShowAds = false;
            // 重新載入一個
            LoadInterstitial();
