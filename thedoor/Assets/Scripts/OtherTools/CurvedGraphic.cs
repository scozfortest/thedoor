...
public class CurvedGraphic : BaseMeshEffect
{
    ...
    [SerializeField]
    protected CurvedGraphic refCUIGraphic;
    public CurvedGraphic RefCUIGraphic
    {
        get
        {
    ...
    [SerializeField]
    protected BezierCurve[] refCurves;
    public BezierCurve[] RefCurves
    {
        get
        {
            return refCurves;
    ...
    public BezierCurve[] EDITOR_RefCurves
    {
        set
        {
            refCurves = value;
        ...
        if (!isCurvesReady)
        {
            ...
            for (int c = 0; c < 2; c++)
            {
                if (refCurves[c] == null)
                {
                    ...
                    curves[c] = go.AddComponent<BezierCurve>();
        ...
        for (int c = 0; c < refCurves.Length; c++)
        {
            ...
            for (int p = 0; p < BezierCurve.CubicBezierCurvePtNum; p++)
            {
                if (curve.ControlPoints != null)
                {
                    ...
                    controlPoints[p].x = rectTrans.rect.width * p / (BezierCurve.CubicBezierCurvePtNum - 1);
