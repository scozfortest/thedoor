...
namespace Scoz.Func {
    public class BluetoothManager {
        ...
        public static void Init(string _deviceName, int _dataLength, Action<BluetoothHelper> _onConnected, Action<BluetoothHelper> _onConnectionFailed,
            ...
            WriteLog.LogFormat("初始化BluetoothManager 目標裝置: {0}", _deviceName);
            //初始化BluetoothHelper
        ...
        static void OnConnected(BluetoothHelper _helper) {
            ...
            WriteLog.LogFormat("已連接至藍芽裝置: {0} 並開始偵聽", _helper.getDeviceName());
            OnConnectedAC?.Invoke(_helper);
        }

        static void OnConnectionFailed(BluetoothHelper _helper) {
            WriteLog.LogFormat("連接藍芽裝置: {0} 失敗", _helper.getDeviceName());
        ...
        public static void ScanNearbyDevices(Action<LinkedList<BluetoothDevice>> _cb) {
            OnScanEndedAC = null;
            if (BTHelper == null) {
                WriteLog.LogFormat("尚未初始化藍芽裝置");
                return;
            }
            WriteLog.LogFormat("嘗試搜尋附近的藍芽裝置");
            bool scanning = BTHelper.ScanNearbyDevices();
            if (scanning) {
                OnScanEndedAC = _cb;
                WriteLog.LogFormat("開始搜尋");
            } else
                WriteLog.LogFormat("搜尋失敗");
        }
        /// <summary>
        /// 連線到目標藍芽裝置
        /// </summary>
        public static void Connect() {
            ...
            WriteLog.LogFormat("嘗試連接至藍芽裝置: {0}", BTHelper.getDeviceName());
            BTHelper.Connect();
        }
        /// <summary>
        /// 中斷目標藍芽裝置
        ...
        public static void Disconnect() {
            ...
            WriteLog.LogFormat("中斷連接藍芽裝置: {0}", BTHelper.getDeviceName());
            BTHelper.Disconnect();
        }
        /// <summary>
        /// 送字串資料到裝置
        ...
        public static void SendData(string _str) {
            ...
            WriteLog.LogFormat("送字串到裝置: " + _str);
        ...
        public static void SendData(byte[] _bytes) {
            ...
            WriteLog.LogFormat("送Bytes到裝置: " + log);
